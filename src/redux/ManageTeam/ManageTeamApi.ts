...
export const getTeamDataApiMAIN = () => { return get<Team>("/team/0");} 
export const updateTeamNameAPI = (teamId: number, name: String) => { return put(`/${teamId}/update/`,{ name }); };

export const getTeamMembersAPI = () => { return get<Member[]>("/member/team/0");} 
export const addTeamMemberAPI = ( id: number) => { post(`/team/0/member`, {id});} 

export const removeTeamMember = (memberId: number) => { return remove(`/team/0/member/${memberId}`)}; 
